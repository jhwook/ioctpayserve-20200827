...
const sendstoken=jdata=>{return new Promise((resolve,reject)=>{
  let {username,rxaddr,amt2sendfloat,amt2sendwei,currency}= jdata  // db.balance.findOne({raw:true,where:{username:username,currency:'ETH'}}).then(respethbal=>{  })
  getbalance({username:username,currency:'ETH'},'float').then(respbal=>{
    ...
    db.balance.findOne({raw:true,where:{username:username,currency:currency}}).then(respacct=>{
...
const init=()=>{
  ...
  db.operations.findOne({raw:true,where:{key_:'GAS_PRICE_TOKEN'}}).then(resp=>{ if(resp && resp['value_']){GAS_PRICE_TOKEN=parseInt(resp['value_'])}  })
  db.operations.findOne({raw:true,where:{key_:'GAS_LIMIT_TOKEN'}}).then(resp=>{ if(resp && resp['value_']){GAS_LIMIT_TOKEN=parseInt(resp['value_'])}  })
